...
namespace WebSite.Common
{
    ...
    public class FileUp
    {
        ...
        private int _MaxSize = 1024 * 1024 * 1024;//最大单个上传文件 (默认)
        private string _FileType = "jpg;jpge;png;gif;doc;xls;txt;rar;zip;pdf;mp3;mp4;mov;flv";//所支持的上传类型用";"隔开 
        ...
        private int _MaxWidth = 2000;//图片最大宽度(像素)
        private int _MaxHeight = 2000;//图片最大高度(像素)
        ...
        private string _WaterText = "三宝地";//设置水印文本内容
        private string _WaterFont = "宋体";//设置水印文本字体
        private int _WaterFontSize = 12;//设置水印字大小
        private string _WaterImage = System.Web.HttpContext.Current.Server.MapPath(".") + "\\Manage_SW\\style\\images\\sbdlogo.png";//图片水印地址
        ...
        /// <param name="imgFile">生成缩略图路径</param>
        /// <param name="Ext">视频后缀</param>
        private static void VideoCatchImg(string fileName, string imgFile,string Ext)
        {
            //
            string strPath = ConfigurationManager.AppSettings["ffmpeg"];
            string ffmpeg = "";
            //HttpContext context = HttpContext.Current;
            if (HttpContext.Current != null)//HttpContext.Current并不是无处不在的1. 定时器的回调。2. Cache的移除通知。3. APM模式下异步完成回调。4. 主动创建线程或者将任务交给线程池来执行。这四种情况可能不存在
            {
                ffmpeg=HttpContext.Current.Server.MapPath(strPath);
            }
            else//以下方法可以直接使用
            {
                ...
                ffmpeg = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, strPath);
            }
            
            //string ffmpeg = HttpContext.Current.Server.MapPath(ConfigurationManager.AppSettings["ffmpeg"]);
            //string ffmpeg1 = HttpContext.Current.Request.MapPath(ConfigurationManager.AppSettings["ffmpeg"]);
            //string ffmpeg2 = HttpRuntime.AppDomainAppPath;
            //string ffmpeg3 = System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, ConfigurationManager.AppSettings["ffmpeg"]);//错误组合模式，要“a\\b.c”这种模式，HttpRuntime.AppDomainAppPath方法没错
            //string ffmpeg4 = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, ConfigurationManager.AppSettings["ffmpeg"]);
            //string ffmpeg5 = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ffmpeg\\ffmpeg.exe");
            //string ffmpeg6 = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, strPath);
            //
            string flv_img = imgFile.Replace(Ext, "jpg");//把mp4后缀改成jpg
            string flv_img2 = fileName.Replace(Ext, "jpg");
            string flv_img3 = fileName.Replace(Ext, "gif");

            string FlvImgSizeSmall = ConfigurationManager.AppSettings["CatchFlvImgSizeSmall"];
            string FlvImgSizeBig = ConfigurationManager.AppSettings["CatchFlvImgSizeBig"];

            //System.Diagnostics.Process imgprocess = new System.Diagnostics.Process();
            //imgprocess.StartInfo.FileName = ffmpeg;
            //imgprocess.StartInfo.Arguments = "    -i " + fileName + " -vframes 30 -y -f gif " + flv_img3;//前30帧转成gif
            //imgprocess.Start();
            //if(imgprocess.HasExited)
            //{
            //    imgprocess.Kill();
            //}
            System.Diagnostics.ProcessStartInfo ImgstartInfo = new System.Diagnostics.ProcessStartInfo(ffmpeg);
            System.Diagnostics.ProcessStartInfo ImgstartInfo2 = new System.Diagnostics.ProcessStartInfo(ffmpeg);
            //System.Diagnostics.ProcessStartInfo ImgstartInfo3 = new System.Diagnostics.ProcessStartInfo(ffmpeg);

            ImgstartInfo.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
            ImgstartInfo2.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;
            //ImgstartInfo3.WindowStyle = System.Diagnostics.ProcessWindowStyle.Hidden;

            ImgstartInfo.Arguments = "  -i  " + fileName + "  -y  -f  image2  -ss 2 -vframes 1  -s  " + FlvImgSizeSmall + " " + flv_img;
            ImgstartInfo2.Arguments = "  -i  " + fileName + "  -y  -f  image2  -ss 2 -vframes 1  -s  " + FlvImgSizeBig + " " + flv_img2;
            //ImgstartInfo3.Arguments = "    -i " + fileName + " -vframes 30 -y -f gif " + flv_img3;//前30帧转成gif
        
            try
            {
                System.Diagnostics.Process.Start(ImgstartInfo);
                System.Diagnostics.Process.Start(ImgstartInfo2);
                //System.Diagnostics.Process.Start(ImgstartInfo3);

                //System.IO.File.Exists(flv_img);
                //System.IO.File.Exists(flv_img2);
            }
            catch
            {
                //return "";
        ...
        private static void AddImageSignText(string imgPath, string filename, string watermarkText, int watermarkStatus, int quality, string fontname, int fontsize)
        {
            ...
            //string imagesurl2 = imgPath.Replace(tmpRootDir, ""); //转换成相对路径
        ...
        public void Upload()
        {
            ...
            string ThumbFileName = FileName.Replace(".", "_t.");//缩略图文件名
            string WaterFileName = FileName.Replace(".", "_w.");//水印图文件名
            ...
            try
            {
                ...
                else if (IsVideo(Ext))//视频生成缩略图
                {
                    VideoCatchImg(_SavePath + FileName, _SavePath + ThumbFileName, Ext);
                }
                //返回
        ...
        #region 评论缩略图
        public void MakeThumbnailImages(string fileName, string newFileName, int maxWidth, int maxHeight)
        {
            byte[] imageBytes = File.ReadAllBytes(fileName);
            System.Drawing.Image img = System.Drawing.Image.FromStream(new System.IO.MemoryStream(imageBytes));
            MakeThumbnailImage(img, newFileName, maxWidth, maxHeight);
        ...
        public void MakeThumbnailImages(string fileName, string newFileName, int width, int height, int mode)
        {
            System.Drawing.Image originalImage = System.Drawing.Image.FromFile(fileName);
            int towidth = width;
            int toheight = height;
