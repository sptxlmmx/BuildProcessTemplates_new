...
{
	/// <summary>
	/// 数据访问类:Dal_OrderActionIntegral
	/// </summary>
	public partial class Dal_OrderActionIntegral
	{
		public Dal_OrderActionIntegral()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "SW_OrderActionIntegral"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from SW_OrderActionIntegral");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebSite.Model.Mod_OrderActionIntegral model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into SW_OrderActionIntegral(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebSite.Model.Mod_OrderActionIntegral model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update SW_OrderActionIntegral set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SW_OrderActionIntegral ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_OrderActionIntegral GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,OrderID,OrderNo,OperatorModel,OperatorID,OperatorName,OrderState,LogisticsState,PayState,Reason,AddDate from SW_OrderActionIntegral ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			WebSite.Model.Mod_OrderActionIntegral model=new WebSite.Model.Mod_OrderActionIntegral();
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_OrderActionIntegral DataRowToModel(DataRow row)
		{
			WebSite.Model.Mod_OrderActionIntegral model=new WebSite.Model.Mod_OrderActionIntegral();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM SW_OrderActionIntegral ");
			if(strWhere.Trim()!="")
			{
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_OrderActionIntegral where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_OrderActionIntegral where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_OrderActionIntegral where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SW_OrderActionIntegral";
