...
namespace WebSite.DAL
{
	...
	/// 数据访问类:Dal_Order
	/// </summary>
	public partial class Dal_Order
	{
		public Dal_Order()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "SW_Order"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from SW_Order");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebSite.Model.Mod_Order model)
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into SW_Order(");
            strSql.Append("Model,OrderNo,ShopID,ShopName,State,Phone,UserID,UserName,TotalCount,TotalWeight,TotalFreight,TotalAmount,ProductAmount,OrderAmount,IsFreeFreight,DiscountReason,Discount,DiscountAmount,LessAmount,AddAmount,CurrencyType,CurrencyMark,Consignee,Country,Province,City,Region,Address,Zipcode,Tel,Mobile,SignBuilding,BestTime,ReturnAmount,ReturnGoods,Replacement,Remark,LogisticsDllName,LogisticsName,LogisticsNo,LogisticsState,LogisticsDate,LogisticsAmount,ReceivingDate,PayState,PayNo,PayName,PayDllName,PayAmount,PayDate,IsInvoice,InvoiceCommercial,Message,AddDate,WebSiteID,SubIntegral,AddIntegral,IntegralAmount,CouponID,CouponNo,CouponAmount,LogisticsCompany,LogisticsCode,CompleteDate)");
            ...
            parameters[63].Value = model.CompleteDate;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebSite.Model.Mod_Order model)
		{
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update SW_Order set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SW_Order ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			{
                //删除子项
                Dal_OrderAction doa = new Dal_OrderAction();
                doa.DeleteWhere(" OrderID=" + ID + " ");
                Dal_OrderDetail dod = new Dal_OrderDetail();
                dod.DeleteWhere(" OrderID=" + ID + " ");
				return true;
			}
			else
			{
		...
		public bool DeleteList(string IDlist )
		{
			...
			if (rows > 0)
			{
                //删除子项
                Dal_OrderAction doa = new Dal_OrderAction();
                doa.DeleteWhere(" OrderID in(" + IDlist + ") ");
                Dal_OrderDetail dod = new Dal_OrderDetail();
                dod.DeleteWhere(" OrderID in(" + IDlist + ") ");
				return true;
			}
			else
			{
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_Order GetModel(int ID)
		{
            ...
            WebSite.Model.Mod_Order model = new WebSite.Model.Mod_Order();
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_Order DataRowToModel(DataRow row)
		{
            WebSite.Model.Mod_Order model = new WebSite.Model.Mod_Order();
            if (row != null)
            {
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_Order where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_Order where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_Order where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
        ...
        /// 得到一个对象实体
        /// </summary>
        public WebSite.Model.Mod_Order GetModel(string OrderNo)
        {
