...
{
	/// <summary>
	/// 数据访问类:Dal_NewsComment
	/// </summary>
	public partial class Dal_NewsComment
	{
		public Dal_NewsComment()
		{}
		#region  BasicMethod
        ...
        public int GetMaxId()
        {
            return DbHelperSQL.GetMaxID("ID", "SW_NewsComment");
        }
        ...
        public bool Exists(int ID)
        {
            ...
            strSql.Append("select count(1) from SW_NewsComment");
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(WebSite.Model.Mod_NewsComment model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into SW_NewsComment(");
            strSql.Append("Model,NewsID,NewsTitle,ReuserID,ReuserName,ReuserNickName,ReuserLevelID,ReuserImage,ReuserClass,UserParentsID,ReplyUserID,ReplyUserName,ReplyNickName,ReplyID,Content1,IsVideo,VideoTime,VideoContent,Explanation,IsReport,ColumnID,ReColumnID,BestThanks,FabulousCount,ReplyCount,State,IsReview,IsBest,IsLook,OrderBy,AddDate,WebSiteID)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@VideoTime", SqlDbType.VarChar,10),
					new SqlParameter("@VideoContent", SqlDbType.NText),
            ...
            parameters[15].Value = model.IsVideo;
            parameters[16].Value = model.VideoTime;
            parameters[17].Value = model.VideoContent;
            parameters[18].Value = model.Explanation;
            parameters[19].Value = model.IsReport;
            parameters[20].Value = model.ColumnID;
            parameters[21].Value = model.ReColumnID;
            parameters[22].Value = model.BestThanks;
            parameters[23].Value = model.FabulousCount;
            parameters[24].Value = model.ReplyCount;
            parameters[25].Value = model.State;
            parameters[26].Value = model.IsReview;
            parameters[27].Value = model.IsBest;
            parameters[28].Value = model.IsLook;
            parameters[29].Value = model.OrderBy;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(WebSite.Model.Mod_NewsComment model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update SW_NewsComment set ");
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from SW_NewsComment ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        /// 得到一个对象实体
        /// </summary>
        public WebSite.Model.Mod_NewsComment GetModel(int ID)
        {

            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,Model,NewsID,NewsTitle,ReuserID,ReuserName,ReuserNickName,ReuserLevelID,ReuserImage,ReuserClass,UserParentsID,ReplyUserID,ReplyUserName,ReplyNickName,ReplyID,Content1,IsVideo,VideoTime,VideoContent,Explanation,IsReport,ColumnID,ReColumnID,BestThanks,FabulousCount,ReplyCount,State,IsReview,IsBest,IsLook,OrderBy,AddDate,WebSiteID from SW_NewsComment ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
            ...
            WebSite.Model.Mod_NewsComment model = new WebSite.Model.Mod_NewsComment();
        ...
        /// 得到一个对象实体
        /// </summary>
        public WebSite.Model.Mod_NewsComment DataRowToModel(DataRow row)
        {
            WebSite.Model.Mod_NewsComment model = new WebSite.Model.Mod_NewsComment();
            if (row != null)
            {
                ...
                if (row["IsVideo"] != null && row["IsVideo"].ToString() != "")
                {
                    model.IsVideo = int.Parse(row["IsVideo"].ToString());
                }
        ...
        public DataSet GetList(string strWhere)
        {
            ...
            strSql.Append(" FROM SW_NewsComment ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_NewsComment where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_NewsComment where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_NewsComment where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SW_NewsComment";
