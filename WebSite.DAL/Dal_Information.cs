...
{
	/// <summary>
	/// 数据访问类:Dal_Information
	/// </summary>
	public partial class Dal_Information
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "SW_Information"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from SW_Information");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebSite.Model.Mod_Information model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into SW_Information(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebSite.Model.Mod_Information model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update SW_Information set ");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(WebSite.Model.Mod_Information model, bool IsSub)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update SW_Information set ");
            strSql.Append("Model=@Model,");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SW_Information ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_Information GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Model,UserID,Title,Content1,Content2,Introduction,TypeID,STypeID,AddDate,ModifyDate,BrowseCount,CommendDate,State,Tags,Image,IsCommend,IsTop,IsEssence,IsPermissions,WebSiteID,Author,Source,CommentCount,UserName,FileURL,FileName,IsCrawl,IsAudit,AuditDate,AuditMember,OrderBy,CitationTitle,SubTitle,Link,TitleColor,IsComment from SW_Information ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			WebSite.Model.Mod_Information model=new WebSite.Model.Mod_Information();
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_Information DataRowToModel(DataRow row)
		{
			WebSite.Model.Mod_Information model=new WebSite.Model.Mod_Information();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM SW_Information ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_Information where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_Information where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_Information where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SW_Information";
        ...
        /// <returns></returns>
        public WebSite.Model.Mod_Information GetNextInfo(WebSite.Model.Mod_Information ModelInfo)
        {
            //按时间来
            ...
            //按ID来
            string strSql = "Select top 1 * from SW_Information WHERE ID>" + ModelInfo.ID + " and ID!=" + ModelInfo.ID + " AND  TypeID=" + ModelInfo.TypeID + " And Model='" + ModelInfo.Model + "' order by ID ASC";
        ...
        /// <param name="ModelInfo"></param>
        /// <returns></returns>
        public WebSite.Model.Mod_Information GetLastInfo(WebSite.Model.Mod_Information ModelInfo)
        {
            //按时间来
            ...
            //按ID来
            string strSql = "Select top 1 * from SW_Information WHERE ID<" + ModelInfo.ID + " and ID!=" + ModelInfo.ID + " AND  TypeID=" + ModelInfo.TypeID + " And Model='" + ModelInfo.Model + "' order by ID DESC";
