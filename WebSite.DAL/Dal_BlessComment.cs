...
{
	/// <summary>
	/// 数据访问类:Dal_BlessComment
	/// </summary>
	public partial class Dal_BlessComment
	{
		public Dal_BlessComment()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "SW_BlessComment"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from SW_BlessComment");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebSite.Model.Mod_BlessComment model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into SW_BlessComment(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebSite.Model.Mod_BlessComment model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update SW_BlessComment set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SW_BlessComment ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_BlessComment GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Model,NewsID,NewsTitle,ReuserID,ReuserName,ReuserNickName,ReuserLevelID,ReuserImage,ReuserClass,UserParentsID,ReplyUserID,ReplyUserName,ReplyNickName,ReplyID,Content1,Explanation,IsReport,ColumnID,ReColumnID,BestThanks,FabulousCount,ReplyCount,State,IsReview,IsBest,IsLook,OrderBy,AddDate,WebSiteID from SW_BlessComment ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			WebSite.Model.Mod_BlessComment model=new WebSite.Model.Mod_BlessComment();
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_BlessComment DataRowToModel(DataRow row)
		{
			WebSite.Model.Mod_BlessComment model=new WebSite.Model.Mod_BlessComment();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM SW_BlessComment ");
			if(strWhere.Trim()!="")
			{
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_BlessComment where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_BlessComment where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_BlessComment where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SW_BlessComment";
        ...
        public DataSet GetListByPageALL(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + "  SW_User.LevelID,SW_User.LevelClass,SW_User.Image,SW_User.PicSmall,SW_User.PicBig,SW_User.NiceName ,SW_User.Model as FModel,SW_User.State as FState,SW_User.WebSiteID as FWebSiteID,SW_User.OrderBy as FOrderBy,SW_BlessComment.* from SW_BlessComment left join SW_User on SW_BlessComment.ReuserID=SW_User.ID where  " + strWhere + "  order by " + orderby + " ";

            }
            else
            {
                strSql = "select top " + endIndex + "  SW_User.LevelID,SW_User.LevelClass,SW_User.Image,SW_User.PicSmall,SW_User.PicBig,SW_User.NiceName ,SW_User.Model as FModel,SW_User.State as FState,SW_User.WebSiteID as FWebSiteID,SW_User.OrderBy as FOrderBy,SW_BlessComment.* from SW_BlessComment left join SW_User on SW_BlessComment.ReuserID=SW_User.ID where  " + strWhere + " and  SW_BlessComment.ID not in ( select top " + endIndex * (startIndex - 1) + " SW_BlessComment.ID from SW_BlessComment where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
            return DbHelperSQL.Query(strSql);
        }
        /// <summary>
