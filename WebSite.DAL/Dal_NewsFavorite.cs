...
{
	/// <summary>
	/// 数据访问类:Dal_NewsFavorite
	/// </summary>
	public partial class Dal_NewsFavorite
	{
		public Dal_NewsFavorite()
		{}
		#region  BasicMethod
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "SW_NewsFavorite"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from SW_NewsFavorite");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebSite.Model.Mod_NewsFavorite model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into SW_NewsFavorite(");
			strSql.Append("Model,UserID,BUserID,BUserName,BUserNickName,BUserImage,BUserClass,NewsID,NewsName,NewsTypeID,NewsTypeName,NewsImage,NewsCommentCount,State,OrderBy,AddDate,WebSiteID)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@NewsName", SqlDbType.VarChar,500),
			...
			parameters[12].Value = model.NewsCommentCount;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebSite.Model.Mod_NewsFavorite model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update SW_NewsFavorite set ");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SW_NewsFavorite ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_NewsFavorite GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Model,UserID,BUserID,BUserName,BUserNickName,BUserImage,BUserClass,NewsID,NewsName,NewsTypeID,NewsTypeName,NewsImage,NewsCommentCount,State,OrderBy,AddDate,WebSiteID from SW_NewsFavorite ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			WebSite.Model.Mod_NewsFavorite model=new WebSite.Model.Mod_NewsFavorite();
		...
		/// 得到一个对象实体
		/// </summary>
		public WebSite.Model.Mod_NewsFavorite DataRowToModel(DataRow row)
		{
			WebSite.Model.Mod_NewsFavorite model=new WebSite.Model.Mod_NewsFavorite();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,Model,UserID,BUserID,BUserName,BUserNickName,BUserImage,BUserClass,NewsID,NewsName,NewsTypeID,NewsTypeName,NewsImage,NewsCommentCount,State,OrderBy,AddDate,WebSiteID ");
			strSql.Append(" FROM SW_NewsFavorite ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,Model,UserID,BUserID,BUserName,BUserNickName,BUserImage,BUserClass,NewsID,NewsName,NewsTypeID,NewsTypeName,NewsImage,NewsCommentCount,State,OrderBy,AddDate,WebSiteID ");
			strSql.Append(" FROM SW_NewsFavorite ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            ...
            if (startIndex <= 1)
            {
                strSql = "select top " + endIndex + " * from SW_NewsFavorite where  " + strWhere + "  order by " + orderby + " ";
            }
            else
            {
                strSql = "select top " + endIndex + " * from SW_NewsFavorite where  " + strWhere + " and  ID not in ( select top " + endIndex * (startIndex - 1) + " ID from SW_NewsFavorite where  " + strWhere + "  order by " + orderby + " ) order by " + orderby + " ";
            }
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SW_NewsFavorite";
